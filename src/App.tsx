...
import { INITIAL_ASSETS, initializePriceHistory, generatePriceUpdate } from '@/lib/market-simulator'
...
import { ChartLine, Wallet, Clock, ArrowsClockwise } from '@phosphor-icons/react'
...
function App() {
  ...
  const [assets, setAssets] = useState<Asset[]>(() => 
    INITIAL_ASSETS.map(asset => ({
      ...asset,
      priceHistory: initializePriceHistory(asset)
    }))
  )
    ...
    const interval = setInterval(() => {
      setAssets(currentAssets => {
        const updated = currentAssets.map(asset => {
          const newPrice = generatePriceUpdate(asset.currentPrice, asset.volatility)
          return {
            ...asset,
        ...
        const priceMap = new Map(updated.map(a => [a.id, a.currentPrice]))
        setPortfolio(current => {
          ...
          return updatePositionPrices(current, priceMap)
        })
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Learn trading with AI-powered forecasting
              </p>
            </div>
